...
public class PlayingScene extends Scene {
    ...
    public void constantScreenMove() { 
        ...
        if (PlayingScene.gameScore > 1.4) { //caps off screen move once difficulty hits >2 to fastest screen move
    ...
    public void constantScreenMoveMethod() {
        xOffset++;
        shiftCounter++;
        if (shiftCounter % LevelManager.widthOfBlocks == 0) { //once a whole tile has been "shown" we shift blocks 
